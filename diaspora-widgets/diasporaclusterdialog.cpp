...
void DiasporaClusterDialog::doDrawCluster()
{
    ...
    QColor colorDL(255,0,0,128); // Preferences::prefsPtr()->colorDiasporaLink();
    colorDL.setAlpha(128);
    QPen penGray = QPen(colorDL);
    QColor cyan(128,128,255);
    QPen penCyan = QPen(cyan );  // QPen(Preferences::prefsPtr()->colorStarOtherLink());
    penGray.setWidthF(8);
    ...
    QFont titleFont("Helvetica",16,900); // Preferences::prefs().fontTitle();
...
void DiasporaClusterDialog::saveCluster()
{
    QString fileName =
            QFileDialog::getSaveFileName(this, tr("Export file as Cluster binary data"),
                                         DiasporaPaths::appDir()+"/"+ _cluster.name()+".cluster",
...
void DiasporaClusterDialog::exportCluster() {
    QString fileName =
            QFileDialog::getSaveFileName(
                ...
                DiasporaPaths::appDir()+ "/" + _cluster.name() + ".svg",
...
void DiasporaClusterDialog::exportClusterToGraphViz(QString expFilename )
{
    QString fileName = expFilename != "" ? expFilename :
            QFileDialog::getSaveFileName(
                    ...
                    DiasporaPaths::appDir()+"/"+_cluster.name()+".dot",
